CKEDITOR.dialog.add('emoji', function (editor) {
    var config = editor.config,
        ...
        images = config.emoji_images,
        columns = config.emoji_columns || 8,
        i;
    ...
    // Build the HTML for the emoji images table.
    var labelId = CKEDITOR.tools.getNextId() + '_emoji_emtions_label';
    var html = [
        '<div>' +
    ...
    for (i = 0; i < size; i++) {
        ...
        var emojiLabelId = 'cke_smile_label_' + i + '_' + CKEDITOR.tools.getNextNumber();
        html.push(
            '<td class="cke_dark_background cke_centered" style="vertical-align: middle;" role="presentation">' +
            '<a href="javascript:void(0)" role="option"', ' aria-posinset="' + ( i + 1 ) + '"', ' aria-setsize="' + size + '"', ' aria-labelledby="' + emojiLabelId + '"',
            ' class="cke_smile cke_hand" tabindex="-1" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );">',
            '<img class="cke_hand" title="', config.emoji_descriptions[i], '"' +
            ' cke_src="', CKEDITOR.tools.htmlEncode(config.emoji_path + images[i]), '" alt="', config.emoji_descriptions[i], '"',
            ' src="', CKEDITOR.tools.htmlEncode(config.emoji_path + images[i]), '"',
            // IE BUG: Below is a workaround to an IE image loading bug to ensure the image sizes are correct.
            ( CKEDITOR.env.ie ? ' onload="this.setAttribute(\'width\', 2); this.removeAttribute(\'width\');" ' : '' ), '>' +
            '<span id="' + emojiLabelId + '" class="cke_voice_label">' + config.emoji_descriptions[i] + '</span>' +
            '</a>', '</td>'
        );
